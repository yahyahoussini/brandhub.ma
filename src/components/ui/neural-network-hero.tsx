...
const fragmentShader = `
  ...
  void main() {
    ...
    vec4 darkColor = cppn_fn(uv, 0.1 * sin(0.3 * iTime), 0.1 * sin(0.69 * iTime), 0.1 * sin(0.44 * iTime));
    // Invert colors for light mode
    gl_FragColor = vec4(1.0 - darkColor.r, 1.0 - darkColor.g, 1.0 - darkColor.b, 1.0);
  }
`;
...
function ShaderBackground() {
  ...
  return (
    <div ref={canvasRef} className="bg-white absolute inset-0 -z-10 w-full h-full" aria-hidden>
      <div 
...
export default function Hero({
  ...
  ctaButtons = [
    ...
    { text: "Voir nos réalisations", href: "#showcase" }
  ],
  microDetails = ["Design minimaliste", "Animations fluides", "Performance optimale"]
}: HeroProps) {
  ...
  return (
    <section ref={sectionRef} className="relative min-h-screen w-full overflow-hidden">
      ...
      <div className="relative mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16">
        <div ref={badgeRef} className="inline-flex items-center gap-2 rounded-full border border-foreground/10 bg-foreground/5 px-3 py-1.5 backdrop-blur-sm">
        ...
        <h1 ref={headerRef} className="max-w-2xl text-left text-5xl font-extralight leading-[1.05] tracking-tight text-foreground sm:text-6xl md:text-7xl">
          {title}
        </h1>
        ...
        <div ref={ctaRef} className="flex flex-wrap items-center gap-3 pt-2">
          {ctaButtons.map((button, index) => (
            <a
              key={index}
              href={button.href}
              className={`rounded-2xl border border-foreground/10 px-5 py-3 text-sm font-light tracking-tight transition-colors focus:outline-none focus:ring-2 focus:ring-foreground/30 duration-300 ${
                button.primary
                  ? "bg-foreground/10 text-foreground backdrop-blur-sm hover:bg-foreground/20"
        ...
        <ul ref={microRef} className="mt-8 flex flex-wrap gap-6 text-xs font-extralight tracking-tight text-foreground/60">
          {microDetails.map((detail, index) => {
            const refMap = [microItem1Ref, microItem2Ref, microItem3Ref];
            return (
              <li key={index} ref={refMap[index]} className="flex items-center gap-2">
                <span className="h-1 w-1 rounded-full bg-foreground/40" /> {detail}
              </li>
            );
          })}
        </ul>
